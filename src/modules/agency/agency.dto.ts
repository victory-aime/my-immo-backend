...
export class createAgencyOwnerDto {
  ...
  @IsString()
  @ApiProperty({ example: 'Agence spécialisée en location résidentielle', description: 'Description de l\'agence' })
  description: string;
  ...
  @IsString()
  @MinLength(12)
  @ApiProperty({ example: 'motdepasse123456', description: 'Mot de passe du propriétaire (min. 12 caractères)', minLength: 12 })
  password: string;
}

export class updateAgencyDto extends createAgencyOwnerDto {
  @IsUUID()
  @ApiProperty({ example: 'uuid-de-l-agence', description: 'Identifiant de l\'agence à mettre à jour' })
  agencyId: string;

  @IsUUID()
  @ApiProperty({ example: 'uuid-du-user', description: 'Identifiant de l\'utilisateur effectuant la mise à jour' })
  userId: string;

  @IsOptional()
  @IsString()
  @ApiPropertyOptional({ example: 'https://res.cloudinary.com/example/logo.png', description: 'URL du logo de l\'agence' })
  agencyLogo?: string;
}